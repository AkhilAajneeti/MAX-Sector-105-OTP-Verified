document.addEventListener("DOMContentLoaded", function () {
  ...
  [
    "ajax-header-contact",
    "ajax-header-contact-2",
    "ajax-header-contact-3",
    "ajax-header-contact-4",
    "ajax-header-contact-5",
    "ajax-header-contact-6",
    "ajax-header-contact-7",
    ...
    otpForm.addEventListener("submit", async function (e) {
      ...
      if (!verifyRes.ok) {
        Swal.fire({
          ...
          // ensure UI + timer present when reopening
          ensureResendElements();
          wireResendButton();
          startResendTimer(30);
          otpModal2.show();
      ...
      // On success: submit lead again, with name prefixed "OTP_"
      const verifiedPayload = {
        page_url: pendingFormData.page_url || window.location.href,
        project_name: pendingFormData.project_name || "Sobha",
        name: `OTP_${pendingFormData.name}`,
        mobile: pendingFormData.mobile || pendingFormData.phone,
      ...
      if (secondSubmit.ok) {
        ...
        if (duplicateDetected) {
          ...
          Swal.fire({
            title: "Verified",
            text: "OTP verified — your contact is already saved.",
            icon: "success",
            timer: 2000,
            showConfirmButton: false,
          });
